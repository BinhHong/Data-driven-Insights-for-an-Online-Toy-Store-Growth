...
- using M Code to create a table `Calendar` starting from 2013-03-19 to 2015-04-02
...
- 3 dimension tables are Calendar, Products and Orders and 2 fact tables are Order Items and Order Item Refunds
...
- Calculated columns:
    - add column `Weekend` for the tables `Order Items`, `Orders`, `Websites Pageview` and `Website Sessions`
    - add column `Years Since Launch` in the table `Products` using **Time Intelligence**
    - add column `Price Tier` in the table `Products`
- Measures:
    - `Quantity Ordered` and `Quantity Sold`
